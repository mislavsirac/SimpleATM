<?php
  ...
  if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    $qry = mysqli_query($conn,"select * from users where cardnumber='$cardnumber'"); // select query
    $reciever = mysqli_fetch_array($qry); // fetch data

    $balancereciever = $reciever['balance'];
    $balancesender = $data['balance'];
    $deposit = $_POST['amount'];
    $balancereciever += $deposit;
    $balancesender -= $deposit;
    $edit = mysqli_query($conn,"update users set balance = '$balancereciever' where cardnumber = '$cardnumber'");
    $edit1 = mysqli_query($conn,"update users set balance = '$balancesender' where ID = '$id'");
                ...
                <td align="center">
                  ...
                  Balance: <?php echo $data['balance']; ?> <br>
                    <form class="index_login_form" method="post" style="margin-top:50px;">
                        ...
                        <div class="form-group"><input id="cardnumber" class="form-control" type="text" name="cardnumber" placeholder="Enter the cardnumber of the recipient" /></div>
                        <div class="form-group"><input id="amount" class="form-control" type="text" name="amount" placeholder="Enter the amount to send" /></div>
                        <div class="form-group"><button class="btn btn-primary btn-block" type="submit">Send money</button></div></form>
